...
import { leasingAPI } from '../services/api';
import toast from 'react-hot-toast';
import { 
    FaCar, FaFileAlt, FaCheckCircle, FaTimesCircle, 
    FaHourglassHalf, FaMoneyBillWave, FaUser, FaPhone, 
    FaMapMarkerAlt, FaCalendarAlt, FaEye, FaSync, FaBuilding,
    FaPercentage
} from 'react-icons/fa';

const LeasingManagerDashboard = () => {
    ...
    const fetchRequests = async () => {
        ...
        try {
            ...
            const response = await leasingAPI.getRequests(params);
    ...
    const fetchStats = async () => {
        try {
            const response = await leasingAPI.getStats();
    ...
    const handleUpdateStatus = async () => {
        ...
        try {
            await leasingAPI.updateStatus(selectedRequest.id, modalData);
            toast.success('✅ تم تحديث حالة طلب التأجير بنجاح');
            setShowModal(false);
            setSelectedRequest(null);
            setModalData({ status: '', approved_amount: '', down_payment: '', duration_years: '', monthly_payment: '', notes: '' });
            fetchRequests();
            fetchStats();
        } catch (error) {
    ...
    const openModal = (request, status) => {
        ...
        setModalData({
            ...
            monthly_payment: request.monthly_installment || '',
            notes: request.notes || ''
        });
        setShowModal(true);
    };
        ...
        <div className="min-h-screen bg-gray-100">
                    ...
                    <div className="flex flex-wrap justify-between items-center">
                        <div>
                            <h1 className="text-3xl font-bold text-white flex items-center gap-2">
                                ...
                                لوحة تحكم مدير التأجير التمويلي
                            </h1>
                            <p className="text-blue-100 mt-1">إدارة طلبات التأجير التمويلي</p>
                        </div>
                        <button
            ...
            <div className="max-w-7xl mx-auto px-4">
                <div className="flex flex-wrap gap-2 mb-6">
                    <button onClick={() => setFilter('all')} className={`px-4 py-2 rounded-lg font-semibold transition ${filter === 'all' ? 'bg-blue-600 text-white' : 'bg-white text-gray-700 border'}`}>الكل</button>
                    ...
                    <div className="overflow-x-auto">
                        <table className="min-w-full divide-y divide-gray-200">
                            ...
                            <tbody className="bg-white divide-y divide-gray-200">
                                {requests.length === 0 ? (
                                    <tr>
                                        <td colSpan="7" className="px-4 py-8 text-center text-gray-500">لا توجد طلبات تأجير</td>
                                    </tr>
                                ) : (
                                    requests.map((req) => (
                ...
                <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
                    <div className="bg-white rounded-2xl shadow-xl max-w-md w-full p-6">
                        <h3 className="text-xl font-bold mb-4">
                            {modalData.status === 'approved' ? '✅ الموافقة على التأجير' : 
                             modalData.status === 'rejected' ? '❌ رفض التأجير' : 
                             '📝 تحديث طلب التأجير'}
                        </h3>
                        ...
                        {(modalData.status === 'approved' || modalData.status === 'under_review') && (
                            <>
                                ...
                                <div className="mb-3">
                                    <label className="block text-gray-700 mb-1">الدفعة المقدمة (%)</label>
                                    <input
                                        type="number"
                                        value={modalData.down_payment}
                                        onChange={(e) => setModalData({...modalData, down_payment: e.target.value})}
                                        className="w-full px-4 py-2 border rounded-lg"
                                        placeholder="مثال: 10"
                                    />
                                </div>
                                <div className="mb-3">
                                    <label className="block text-gray-700 mb-1">مدة العقد (سنوات)</label>
                                    <input
                                        type="number"
                                ...
                                <div className="mb-3">
                                    <label className="block text-gray-700 mb-1">القسط الشهري (دينار)</label>
                                    <input
                                        type="number"
                                        value={modalData.monthly_payment}
                                        onChange={(e) => setModalData({...modalData, monthly_payment: e.target.value})}
                                        className="w-full px-4 py-2 border rounded-lg"
...
export default LeasingManagerDashboard;