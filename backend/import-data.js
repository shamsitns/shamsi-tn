...
async function importData() {
  ...
  try {
    ...
    // استيراد العملاء (Leads)
    const leads = JSON.parse(fs.readFileSync('data-leads.json', 'utf8'));
    for (const l of leads) {
      await db.query(
        `INSERT INTO leads (
          id, user_name, phone, city, property_type, payment_method, monthly_bill, roof_area, roof_direction,
          shading, required_kw, estimated_price, panels, panel_power, annual_production, annual_savings,
          payback_years, commission, status, created_at
